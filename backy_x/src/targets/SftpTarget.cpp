...
bool SftpTarget::sendFile(const std::string& relativePath, const FileMetadata& remoteRelativePath) {
    ...
    if (path_qs.startsWith("/")) {
        path_qs.remove(0, 1);  // éviter les "//"
    }

    // Construire proprement l’URL
    // Note: m_remoteBasePath is not used in this new logic directly for URL construction.
    // The expectation is that remoteRelativePath is now a full path from server root,
    // or a path relative to a base that is implicitly part of the SFTP user's default directory.
    // The original buildSftpUrl used m_remoteBasePath. This new logic does not.
    // This might change where files are uploaded if remoteRelativePath was previously relative to m_remoteBasePath.
    // The instruction says "path.remove(0,1)" to avoid "//", implying host:port/path.
    // If remoteRelativePath was "/foo/bar.txt", path_qs becomes "foo/bar.txt".
    // If remoteRelativePath was "foo/bar.txt", path_qs stays "foo/bar.txt".
    // URL becomes sftp://host:port/foo/bar.txt
    QString url_qstring = QString("sftp://%1:%2/%3").arg(host_qs).arg(m_port).arg(path_qs);

    // Log temporaire
    qDebug() << "SFTP URL generated by new logic: " << url_qstring;

    // Vérification minimale
    if (!url_qstring.startsWith("sftp://")) {
        qWarning() << "Malformed URL generated by new logic: " << url_qstring;
    }
